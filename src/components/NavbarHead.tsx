import React from 'react'

const NavbarHead: React.FC = () => {
  return (
    <div className="bg-yellow-400 w-full "style={{height:'50px'}}>
      
      <div className="flex justify-between items-center ml-16  mr-16  ">
        <div className='flex items-center'>
          <div className='flex items-center'>
         <img className='w-16 p-4' src="data:image/png;base64," alt="" />
         <p className='text-black font-bold'>PAK</p>
         </div>
         <div className='border-r-2 w-4 border-black-500 h-8'></div>
         <div className='flex items-center  ml-4'>
          <i className='text-black font-bold underline  '>SPORTZ SHOP</i>
          </div>

        </div>


        {/* CENTER   */}
        <div>
          <div className='flex items-center'>
            <i className='text-yellow-500 bg-neutral-950  text-center font-bold' style={{height:'28px', width: "50px"}}>Live</i>
            <p className='text-center ml-2 text-lg font-medium'>Legends Faceoff : Encode vs Eagles</p>
          </div>
          
        </div>
        <div>
          <i className="underline font-bold">Live Event Pass</i>
        </div>
        </div>
      </div>
      
  )
}

export default NavbarHead